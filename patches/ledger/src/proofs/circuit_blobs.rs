...
pub fn fetch_blocking(filename: &impl AsRef<Path>) -> std::io::Result<Vec<u8>> {
    ...
    // Network fetch disabled — not available in SP1 guest.
    // Set MINA_CIRCUIT_BLOBS_BASE_DIR to point to a local clone of circuit-blobs.
    Err(to_io_err("network fetch disabled: set MINA_CIRCUIT_BLOBS_BASE_DIR"))
}
